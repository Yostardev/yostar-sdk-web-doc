...
|:---------:|:----:|:---------------------------------------------:|
| requestId | 字符串  |                   绑定操作的请求id                   |
|  lastName  | 字符串  |  账户持有人（姓,仅限日语片假名,需要接入时前端直接校验），长度限制 双字节 60 字节  |
|  firstName  | 字符串  | 账户持有人（名,仅限日语片假名,需要前端接入时前端直接校验），长度限制 双字节 60 字节 |
...
|  参数  | 类型  |                                                    说明                                                    |
| :----: |:---:|:--------------------------------------------------------------------------------------------------------:|
| result | 整数  | 0：等待用户银行信息注册完成，需要调用<br/>[银行绑定获取手机号银行信息](#银行绑定获取手机号银行信息)展示最终绑定注册结果<br>1：请求id不存在<br>2:银行卡信息未填<br>3：发起GMO注册API错误 |
